...
waypoints = loadmat('RV_waypoints.mat')['waypoints']
...
for i in range(0, num_of_wps-1):
    ...
    xref0, xref = min(x0, x1), max(x0, x1)
    yref0, yref = min(y0, y1), max(y0, y1)
    zref0, zref = min(z0, z1), max(z0, z1)
    psiref0, psiref = min(psi0, psi1), max(psi0, psi1)
    ...
    phase.add_state('w', fix_initial=True, fix_final=False, units='m/s', rate_source='wdot',
                    lower = -wlimit, upper = wlimit, ref0 = -wref, ref = wref)
    ...
    phase.add_state('psi', fix_initial=True, fix_final=True, units='rad', rate_source='psidot',
                    lower = -np.pi, upper = np.pi, ref0 = psiref0, ref = psiref)
    ...
    phase.add_state('r', fix_initial=True, fix_final=False, units='rad/s', rate_source='rdot',
                    lower = -1/2*np.pi/2, upper = 1/2*np.pi/2, ref0 = -rref, ref = rref)
    # if i == 0 :
    #     phase.set_time_options(fix_initial=True, units='s', duration_ref0 = 0, 
    ...
    # add controls
    phase.add_control('n1', units = '1/s', opt=True, lower = 0.0, upper = 500.0,
                       rate_continuity=True)
    phase.add_control('n2', units = '1/s', opt=True, lower = 0.0, upper = 500.0,
                       rate_continuity=True)
    phase.add_control('n3', units = '1/s', opt=True, lower = 0.0, upper = 500.0,
                       rate_continuity=True)
    phase.add_control('n4', units = '1/s', opt=True, lower = 0.0, upper = 500.0,
                       rate_continuity=True)
    ...
    p.set_val('traj.'+phase_name+'.t_duration', current_time+1.0, units='s')
...
# # for i in range(num_of_wps-1):
    ...
    error = np.linalg.norm([x0 - x1, y0 - y1, z0-z1])
