...
for i in range(4):
    ...
    # add states
    phase.set_time_options(fix_initial=True, units='s', duration_bounds=(2.0, 10.0),duration_ref0 = 0.0, duration_ref=10.0)
    
    phase.add_state('x', fix_initial=True, fix_final=False, units='m', rate_source='xdot', 
                    lower = -2.0, upper = 36.0, ref0 = 0.0, ref = 20.0)
    
    phase.add_state('y', fix_initial=True, fix_final=False, units='m', rate_source='ydot',
                    lower = -18.0, upper = 14.0, ref0 = -10.0, ref = 10.0)
    
    phase.add_state('z', fix_initial=True, fix_final=False, units='m', rate_source='zdot',
...
for i in range(4):
    ...
    p.set_val('traj.'+phase_name+'.t_initial', 0, units='s')
    p.set_val('traj.'+phase_name+'.t_duration', 10.0, units='s')
    ...
    # p.set_val('traj.'+phase_name+'.states:x', phase.interp('x', [0.0, 9.9688]), units = 'm')
    # p.set_val('traj.'+phase_name+'.states:y', phase.interp('y',[0.0, -0.6902]), units = 'm')
    # p.set_val('traj.'+phase_name+'.states:z', phase.interp('z', [-2.0, -1.733]), units = 'm')
...
t_sol = p.get_val('traj.phase1.timeseries.time')
J = p.get_val('traj.phase2.timeseries.states:J')
...
t_sim = sim.get_val('traj.phase2.timeseries.time')
...
x = sim.get_val('traj.phase4.timeseries.states:x')
y = sim.get_val('traj.phase4.timeseries.states:y')
z = sim.get_val('traj.phase4.timeseries.states:z')
psi = sim.get_val('traj.phase4.timeseries.states:psi')
